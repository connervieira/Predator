...
def manage_time_offset(): # This function watches the system time, and reset the time offset if the system time changes.
    ...
    while global_variables.PREDATOR_RUNNING: # Run forever.
        ...
        if (abs(difference_time) > 5): # Check to see if the system time changed by more than 5 seconds.
...
def stop_predator():
    ...
    for i in range(0, 20):
        time.sleep(0.1)
        if (len(threading.enumerate()) <= 1): # Check to see if only one thread is still running (the main thread)
            break
    if (len(threading.enumerate()) > 1): # Check to see if there are still background threads running.
        display_message("The following threads have not exited properly. Press Ctrl+C again to force terminate.", 2)
