...
def alpr_stream(device):
    utils.debug_message("Starting ALPR stream")
...
def alpr_stream_maintainer(): # This function runs an endless loop that maintains
    ...
    while global_variables.PREDATOR_RUNNING:
        utils.debug_message("Starting ALPR stream maintainance cycle")
        ...
        utils.save_to_file(alpr_stream_file_location, "") # Erase the contents of the ALPR stream file.

        for message in stream_file_contents: # Iterate through each line in the loaded stream file contents.
            if (utils.is_json(message) == True):
                ...
                if ("error" in message): # Check to see if there were errors while executing the ALPR process. This will only work for alerts issued by Phantom, not OpenALPR.
                    utils.display_message("Phantom ALPR encountered an error: " + message["error"], 2) # Display the ALPR error.
                ...
                utils.display_message("The information returned by the ALPR engine is not valid JSON. Maybe you've specified the wrong ALPR engine in the configuration?", 2)
        ...
        if (utils.get_time() - last_message_received > 3): # Check to see if a certain number of seconds have passed without receiving any messages from the ALPR process.
            utils.display_message("The ALPR stream hasn't received any ALPR messages in several seconds. The ALPR process may not be running.", 2)
...
def start_alpr_stream(): # This function starts the ALPR stream threads.
    ...
    for device in config["realtime"]["image"]["camera"]["devices"]: # Iterate through each device in the configuration.
        utils.debug_message("Starting ALPR stream " + str(alpr_stream_count))
...
def load_alpr_log():
    global config
    utils.debug_message("Loading license plate history")
    ...
    if (os.path.exists(plate_log_file_location) == False): # If the plate log file doesn't exist, create it.
        utils.save_to_file(plate_log_file_location, "{}") # Save a blank placeholder dictionary to the plate log file.
    ...
    if (utils.is_json(plate_log_file_contents) == True): # If the plate file contains valid JSON data, then load it.
...
def display_alerts(active_alerts):
    for alert in active_alerts: # Iterate through each active alert.
        ...
        print(utils.style.yellow + utils.style.bold)
        print("===================")
        ...
        print("===================")
        print(utils.style.end + utils.style.end)
...
def load_alert_database_remote(source, cache_directory):
    utils.debug_message("Loading remote database source.")
    ...
    if (config["developer"]["offline"] == False): # Check to see if offline mode is disabled.
        try:
            ...
            raw_download_data = "{}"
            utils.display_message("The license plate alert database from " + source + " could not be loaded.", 2)
            if (config["realtime"]["saving"]["remote_alert_sources"]["enabled"] == True):
                if (os.path.exists(cache_directory + "/" + source_hash + ".json")): # Check to see if the cached file exists.
                    utils.debug_message("Attempting to load locally cached data for this remote source.")
        ...
        try:
            ...
            if (config["realtime"]["saving"]["remote_alert_sources"]["enabled"] == True):
                ...
                utils.save_to_file(cache_directory + "/" + source_hash + ".json", json.dumps(alert_database))
        except:
            alert_database = {}
            utils.display_message("The license plate alert database returned by the remote source " + source + " doesn't appear to be compatible JSON data. This source has not been loaded.", 2)
        ...
        utils.display_message("A remote alert database source " + source + " was specified, but Predator is in offline mode. This source has not been loaded.", 2)

    return alert_database

def load_alert_database_local(source):
    utils.debug_message("Loading local database source.")
    if (os.path.exists(source)): # Check to see if the database specified by the user actually exists.
        ...
        if (file_contents[0] == "{"): # Check to see if the first character in the file indicates that this alert database is a JSON database.
            ...
            alert_database = {}
            utils.display_message("The alert database specified at " + source + " does appear to contain compatible JSON data. This source has not been loaded.", 3)
        f.close() # Close the file.
    else: # If the alert database specified by the user does not exist, alert the user of the error.
        alert_database = {}
        utils.display_message("The alert database specified at " + source + " does not exist. This source has not been loaded.", 3)
...
def load_alert_database(sources, project_directory): # This function compiles the provided list of sources into a single complete alert dictionary.
    ...
    utils.debug_message("Loading license plate alert list")
    ...
    utils.save_to_file(config["general"]["interface_directory"] + "/hotlist.json", json.dumps(complete_alert_database, indent=4)) # Save the active alert database to the interface directory.
